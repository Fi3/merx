...
macro_rules! get_fixed {
    () => {
            ...
            ($len:ident, $ty_:ty) => {
                impl TryFrom<i128> for ByteArray<$len> {
                    ...
                    fn try_from(value: i128) -> Result<ByteArray<$len>, Self::Error> {
                        if value <= <$ty_>::max_value() as i128 && value >= <$ty_>::min_value() as i128 {
        ...
        macro_rules! fixed_try_from_parts {
            ($len:ident) => {
                impl<const MAX: i128, const POW: u128> TryFrom<(i128, u128)> for Fixed<$len, MAX, POW> {
                    type Error = ();
        ...
        macro_rules! fixed_try_from_float_rounded {
            ($len:ident) => {
                impl<
                    const MAX: i128,
                    const POW: u128
                    > TryFrom<(f64, FloatRounding)> for Fixed<$len, MAX, POW> {
        
                    type Error = ();
                
                    fn try_from(value: (f64, FloatRounding)) -> Result<Fixed<$len, MAX, POW>, Self::Error> {
                        let frac = (POW as f64).log10() as u8;
                        match value.1 {
                            FloatRounding::Up => Self::try_from(round_value_up(MAX as u128, frac, value.0)
                                                                .ok_or(())?),
                            FloatRounding::Down => Self::try_from(round_value_down(MAX as u128, frac, value.0)
                                                                  .ok_or(())?),
                        }
                    }
                }
